...
Message::Message(QWidget *parent) : QWidget(parent)
{
    ...
    m_font.setFamily("MicrosoftYaHei");
    m_font.setPointSize(12);

    this->setFont(m_font);
    m_leftPixmap = QPixmap("");
    m_rightPixmap = QPixmap(":/Icon/Resources/head.jpg");

    m_loadingMovie = new QMovie(this);
    m_loadingMovie->setFileName(":/Icon/Resources/icon/loading.gif");
...
void Message::setText(QString text, QString time, QSize allSize, User_Type user)
{
    m_msg = text;
    m_time = time;
...
QSize Message::fontRect(QString str)
{
    m_msg = str;
    int minHei = 30;
    int iconWH = 40;
    ...
    kWidth = this->width() - kuangTMP - 2*(iconWH+iconSpaceW+iconRectW);
    textWidth = kWidth - 2*textSpaceRect;
    spaceWid = this->width() - textWidth;
    ...
    m_sanjiaoLeftRect = QRect(iconWH+iconSpaceW+iconRectW, lineHeight/2, sanJiaoW, hei - lineHeight);
    m_sanjiaoRightRect = QRect(this->width() - iconRectW - iconWH - iconSpaceW - sanJiaoW, lineHeight/2, sanJiaoW, hei - lineHeight);

    if(size.width() < (textWidth + spaceWid)) {
        m_kuangLeftRect.setRect(m_sanjiaoLeftRect.x()+m_sanjiaoLeftRect.width(), lineHeight/4*3, size.width()-spaceWid+2*textSpaceRect, hei-lineHeight);
        m_kuangRightRect.setRect(this->width() - size.width() + spaceWid - 2*textSpaceRect - iconWH - iconSpaceW - iconRectW - sanJiaoW,
                                 lineHeight/4*3, size.width()-spaceWid+2*textSpaceRect, hei-lineHeight);
    } else {
        m_kuangLeftRect.setRect(m_sanjiaoLeftRect.x()+m_sanjiaoLeftRect.width(), lineHeight/4*3, kWidth, hei-lineHeight);
        m_kuangRightRect.setRect(iconWH + kuangTMP + iconSpaceW + iconRectW - sanJiaoW, lineHeight/4*3, kWidth, hei-lineHeight);
    }
...
QSize Message::getRealString(QString str)
{
    ...
    return QSize(nMaxWidth+spaceWid, (nCount + 1) * lineHeight+2*lineHeight);
}
...
void Message::paintEvent(QPaintEvent *event)
{
    ...
    if(m_user == User_Type::User_Other) { // 用户
        //头像
//        painter.drawRoundedRect(m_iconLeftRect,m_iconLeftRect.width(),m_iconLeftRect.height());
    ...
    }  else if(m_user == User_Type::User_Me) { // 自己
        //头像
//        painter.drawRoundedRect(m_iconRightRect,m_iconRightRect.width(),m_iconRightRect.height());
    ...
    }  else if(m_user == User_Type::User_Time) { // 时间
        QPen penText;
